...
public class SolrRingIndexer implements XWikiRingIndexer
{
     ...
     * only the ring properties, if second parameter is false. In all cases, the standard document fields (ID, TYPE,
            ...
            boolean withAllPropertiesFromIndex) throws RingException
    {
        ...
        try {
            // We copy all existing field/value pairs starting with the prefix 'property.ring'
            ...
            if (solrDocument != null) {
                ...
                for (String fieldName : solrDocument.getFieldNames()) {
                    if (withAllPropertiesFromIndex && !VERSION_FIELDS.contains(fieldName)) {
                        ...
                        // Only the property ring properties are initialized, the other ones will be added
                        // by the caller.
                        Object fieldValue = solrDocument.getFieldValue(fieldName);
                        solrInputDocument.setField(fieldName, fieldValue);
                    }
    ...
    public void index(DocumentReference vertex)
    {
        ...
        if (vertex != null) {
            ...
            /*try {
                List<XWikiRing> edges = getRingsFrom(vertex);
                for (XWikiRing edge : edges) {
                    if (!serialize(edge.getRelatum()).equals(serialize(getIdentifier(TYPE_TERM_NAME)))) {
                        XWikiRelation relation = getRelation(edge.getRelation());
                        if (edge.hasRelatum() && relation.isTransitive()) {
                            index(edge.getRelatum());
                        }
                    }
                }
     ...
     * XWiki.RingSet.IsA and has-destination = wiki.Book will become, in Solr index: property.ring.XWiki.RingSet.IsA:[wiki.Book].
     */
    public void index(Ring<DocumentReference> ring, boolean transitively) throws RingException
    {
        // Index ring only if the relation is not empty.
        if (ring != null && ring.hasRelation()) {
            try {
                logger.debug("Add ring: {}", ring);
                ...
                if (ring.hasRelatum()) {
                    ...
                    // Add inverse ring, using the IS_CONNECTED_TO relation, so it's not the exact inverse
                    ...
                    /*if (transitively) {
                        XWikiRelation relation = ring.getRelation(ring.getRelation());
                        if (relation.isTransitive()) {
                            List<XWikiRing> secondLevelEdges =
                                    ring.getRingsFrom(ring.getRelatum(), ring.getRelation());
                            for (XWikiRing secondLevelEdge : secondLevelEdges) {
                                XWikiRing edgeByTransitivity =
                                        new BaseXWikiRing(ring.getReferent(), ring.getRelation(),
                                                secondLevelEdge.getRelatum(), ring);
                                ...
                                // nature.
                                if (!(ring.getRelation()
                                        .equals(ring.getIdentifier(BaseXWikiRingSet.IS_A_RELATION_NAME))
                                        && secondLevelEdge.getRelatum()
                                        .equals(ring.getIdentifier(BaseXWikiRingSet.TYPE_TERM_NAME))))
                                {
                ...
                logger.error("Index ring {}", ring, e);
                throw new RingException(e);
            }
        }
    }
    ...
    {
        logger.debug("Remove ring from index: {}", ring);
        // If the ring has no relation, it has no custom index (see #index), so there is no need to remove any
        // index entry.
        if (ring != null && ring.hasRelation() && ring.hasRelatum()) {
            try {
                ...
                if (!ring.getRelation().equals(factory.getIdentifier(Names.IS_CONNECTED_TO_RELATION_NAME))) {
                    // TODO: the value should be removed only if this was the single ring with this destination
...
//                unindexValue(predecessorDocument, ring.serializ(destination));
